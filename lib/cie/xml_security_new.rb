...
module Cie
  module XMLSecurityNew
    ...
    class Document < BaseDocument
      ...
      def sign_document(private_key, certificate, signature_method = RSA_SHA256, digest_method = SHA256)
        noko = Nokogiri::XML(self.to_s) do |config|
          config.options = Cie::XMLSecurityNew::BaseDocument::NOKOGIRI_OPTIONS
        end
    ...
    class SignedDocument < BaseDocument
      include Cie::Saml::ErrorHandling
      ...
      def validate_document(idp_cert_fingerprint, soft = true, options = {})
        ...
        if cert_element
          ...
          if options[:fingerprint_alg]
            fingerprint_alg = Cie::XMLSecurityNew::BaseDocument.new.algorithm(options[:fingerprint_alg]).new
          else
