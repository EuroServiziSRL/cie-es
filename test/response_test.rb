...
class RubySamlTest < Test::Unit::TestCase
  ...
  context "Response" do
    should "raise an exception when response is initialized with nil" do
      assert_raises(ArgumentError) { Cie::Saml::Response.new(nil) }
    end

    should "be able to parse a document which contains ampersands" do
      Cie::XMLSecurity::SignedDocument.any_instance.stubs(:digests_match?).returns(true)
      Cie::Saml::Response.any_instance.stubs(:validate_conditions).returns(true)

      response = Cie::Saml::Response.new(ampersands_response)
    ...
    should "adapt namespace" do
      response = Cie::Saml::Response.new(response_document)
      assert !response.name_id.nil?
      response = Cie::Saml::Response.new(response_document_2)
      assert !response.name_id.nil?
      response = Cie::Saml::Response.new(response_document_3)
      assert !response.name_id.nil?
    end

    should "default to raw input when a response is not Base64 encoded" do
      decoded  = Base64.decode64(response_document_2)
      response = Cie::Saml::Response.new(decoded)
      assert response.document
    end

    context "Assertion" do
      should "only retreive an assertion with an ID that matches the signature's reference URI" do
        response = Cie::Saml::Response.new(wrapped_response_2)
    ...
    context "#validate!" do
      should "raise when encountering a condition that prevents the document from being valid" do
        response = Cie::Saml::Response.new(response_document)
        assert_raise(Cie::Saml::ValidationError) do
          response.validate!
        end
      end
    end
    ...
    context "#is_valid?" do
      should "return false when response is initialized with blank data" do
        response = Cie::Saml::Response.new('')
      ...
      should "return true when the response is initialized with valid data" do
        response = Cie::Saml::Response.new(response_document_4)
        response.stubs(:conditions).returns(nil)
        assert !response.is_valid?
        settings = Cie::Saml::Settings.new
        assert !response.is_valid?
      ...
      should "not allow signature wrapping attack" do
        ...
        response.settings = settings
        assert response.is_valid?
        assert response.name_id == "test@Cie.com"
      end

      should "support dynamic namespace resolution on signature elements" do
        response = Cie::Saml::Response.new(fixture("no_signature_ns.xml"))
        response.stubs(:conditions).returns(nil)
        settings = Cie::Saml::Settings.new
        response.settings = settings
        settings.idp_cert_fingerprint = "28:74:9B:E8:1F:E8:10:9C:A8:7C:A9:C3:E3:C5:01:6C:92:1C:B4:BA"
        Cie::XMLSecurity::SignedDocument.any_instance.expects(:validate_doc).returns(true)
        assert response.validate!
      end

      should "validate ADFS assertions" do
        response = Cie::Saml::Response.new(fixture(:adfs_response_sha256))
        response.stubs(:conditions).returns(nil)
        settings = Cie::Saml::Settings.new
        settings.idp_cert_fingerprint = "28:74:9B:E8:1F:E8:10:9C:A8:7C:A9:C3:E3:C5:01:6C:92:1C:B4:BA"
        response.settings = settings
      ...
      should "validate SAML 2.0 XML structure" do
        ...
        response = Cie::Saml::Response.new(Base64.encode64(resp_xml))
        response.stubs(:conditions).returns(nil)
        settings = Cie::Saml::Settings.new
        settings.idp_cert_fingerprint = signature_fingerprint_1
        response.settings = settings
        assert_raises(Cie::Saml::ValidationError, 'Digest mismatch'){ response.validate! }
      end
    end
    ...
    context "#name_id" do
      should "extract the value of the name id element" do
        response = Cie::Saml::Response.new(response_document)
        assert_equal "support@Cie.com", response.name_id
      ...
      should "be extractable from an OpenSAML response" do
        response = Cie::Saml::Response.new(fixture(:open_saml))
        assert_equal "someone@example.org", response.name_id
      end

      should "be extractable from a Simple SAML PHP response" do
        response = Cie::Saml::Response.new(fixture(:simple_saml_php))
        assert_equal "someone@example.com", response.name_id
      end
    end
    ...
    context "#check_conditions" do
      should "check time conditions" do
        ...
        response = Cie::Saml::Response.new(response_document_6)
        assert response.send(:validate_conditions, true)
        time     = Time.parse("2011-06-14T18:25:01.516Z")
        Time.stubs(:now).returns(time)
        response = Cie::Saml::Response.new(response_document_5)
        assert response.send(:validate_conditions, true)
      end
    end
    ...
    context "#issuer" do
      should "return the issuer inside the response assertion" do
        response = Cie::Saml::Response.new(response_document)
        assert_equal "https://app.Cie.com/saml/metadata/13590", response.issuer
      end
